...
const govukChildDirectories = dir => {
  return fs.readdirSync(dir)
    .filter(file => fs.statSync(path.join(configPaths.govuk_components, file)).isDirectory())
}
...
exports.allComponents = childDirectories(configPaths.components).filter(function (directory) {
  return directory !== 'govuk' && directory !== 'custom'
})
exports.allGovukComponents = govukChildDirectories(configPaths.govuk_components)
...
const getGovukComponentData = componentName => {
  try {
    const yamlPath = path.join(configPaths.govuk_components, componentName, `${componentName}.yaml`)
    return yaml.safeLoad(
      fs.readFileSync(yamlPath, 'utf8'), { json: true }
    )
  } catch (error) {
...
exports.getGovukComponentData = getGovukComponentData
